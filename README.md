...
Continuity: review residual, GSD delivery, and TSG/ADR capability are three lifecycles (`prd/architecture/review-cases/continuity-contract.md`; L_capability board: `prd/architecture/capability-promotion-board.md`). CTV structural apply S3: `apply_industrial_op` in ln-temporal (not legal CTV product). NormativeState force S2–S3: `resolve_force_status_at` (not applicability/corpus). KB ontology YAML FSM + catalog kinds: `kb-ontology.yaml` + `try_push_node` + `prd/architecture/kb-ontology-requirements.md` (not production schema). Reviews 5-6 (`doc/review/review-14-08-2026.md`, `review-15-08-2026.md`) compare law-nexus with de Martim v5 (arXiv:2506.07853v5): wider on clocks/force/cross-act, gap on text reconstruction now closed at prototype. `assembly_fsm` current `S_verify`; oracle diff drift=0 on 402-FZ. Readiness FSM remains `O2_calendar_ordinal`. (pointers, not duplicates)
...
- **ADRs:** [`doc/adr/`](doc/adr/) — MADR-format with mandatory D098 lifecycle
  ...
  (ADR-0016..0027): identity, CTV, force, hierarchy, practice, risk, profiles, applicability, review-case (ADR-0024), consultant-parser (ADR-0025), ruvector-memory (ADR-0026), multi-layer-classifier (ADR-0027)
