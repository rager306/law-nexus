...
Continuity: review residual, GSD delivery, and TSG/ADR capability are three lifecycles (`prd/architecture/review-cases/continuity-contract.md`; L_capability board: `prd/architecture/capability-promotion-board.md`). CTV structural apply S3: `apply_industrial_op` in ln-temporal (not legal CTV product). NormativeState force S2–S3: `resolve_force_status_at` (not applicability/corpus). KB ontology YAML FSM + catalog kinds: `kb-ontology.yaml` + `try_push_node` + `prd/architecture/kb-ontology-requirements.md` (not production schema). Review 4 L0 (`doc/review/review-13-08-2026.md`) names temporal-AST assembly vocabulary; `assembly_fsm` current `S_commit` appends admitted drafts to VersionedMembershipLog, then folds to StructuralAst (402-FZ: 4 roots / 37 nodes). Scoped `kb-hierarchy-registry.yaml` binds 435-FZ (forest) and 402-FZ (chapter tree). Readiness FSM remains `O2_calendar_ordinal`. (pointers, not duplicates)
