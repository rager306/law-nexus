...
[FOUNDATION — COMPLETE]
  ...
  Five-clock temporal model (ADR-0009) + evidence kernel (ADR-0010) [validated]
...
[TEMPORAL LEGAL ONTOLOGY — DESIGN ONLY]
  ADR-0016..0022 L1→L7 (FRBR/LRMoo → CTV → NormativeState → hierarchy/conflict
  → practice → transitional/risk → industry profiles)                 [proposed]
...
- **Temporal legal ontology (design spine, all `[proposed]`):** seven progressive
  layers L1→L7 — FRBR/LRMoo structural identity → Component Temporal Versioning
  (event-sourced, LRMoo v5) → NormativeState → hierarchy/conflict → practice
  overlay → transitional/risk → industry profiles. See ADR-0016..0022.
...
uv run python -m law_nexus_harness.governor     # 37 checks + advisory probes
