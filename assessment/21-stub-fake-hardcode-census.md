...
Lifecycle обеих строк класса D — `addressed-in-draft`: ячейки перевёрнуты настоящей правкой; tracked-evidence-триада disposition protocol замыкается только когда сам регистр входит в tracked-коммит (волна за S04, включая reindex). Вердикт `verified-closed` здесь сознательно не ставится: он означал бы пруф против ревизии, в которой собственно вывод ещё отсутствует.
...
| W3 = S03 | M185-wuj4zf/S03 | Класс D: единый конфиг-вход для `CONSULTANT_EXPORT_DIR` — тестовые сиды CID-D-01 + harness-консюмер CID-D-02; зеркало в `.env.example` уже есть (:3-7); только перенос конфигурации, без изменения логики; уточнение комментария `.env.example`:4 («product CLI») при переносе. **Исполнение:** перенос выполнен configuration-only в S03 (T01 harness-helper, T02 зеркало документа + pin-тест, T03 семь сидов); строки CID-D переведены в `addressed-in-draft` (§Class D, §Closeout S03); вердикт `verified-closed` остаётся за волной после tracked-коммита самого регистра (S04). Литералы fail-closed дефолтов, которые S03 не поднимает, остаются M186+ и переклассифицируются следующим census-sweep |
...
Итог класса D после W3: обе строки — `addressed-in-draft`. Честная оговорка: код переноса уже закоммичен (HEAD `f5798366`, dirty 0 на момент правки), однако переворот lifecycle'а сам становится tracked-evidence только после попадания регистра в коммит S04: ставить `verified-closed` в том же изменении, которое этот вывод впервые предъявляет, значило бы цитировать frozen revision, ещё не существующую. Вердикт остаётся достижимым будущим census-sweep после цикла S04.
