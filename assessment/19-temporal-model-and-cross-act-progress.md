...
| P-05 | GitNexus reindex после closeout-коммитов не сделан (MEM1135); owned by M186 S04 T03 / reindex after S04 tracked commit — **closed after reindex @8097889** (2026-08-28, M186 S04 T03): `analyze --force --name law-nexus` exit 0, meta lastCommit == HEAD 8097889, preflight `gitnexus-index-freshness` pass 8/0; повторный reindex после docs-only P-05/battery-коммитов этого же таска оставляет индекс на финальном HEAD волны | Impact/detect_changes на stale index | `node .gitnexus/run.cjs analyze --force --name law-nexus` после tracked-коммита |
