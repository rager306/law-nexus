...
- Local 2026-05-10 smoke evidence confirmed GraphBLAS-backed FalkorDB startup and procedure discovery on `falkordb/falkordb:edge`. Bounded algorithm fixtures confirmed output shape for `algo.pageRank('Page', 'LINKS')`, `algo.WCC({nodeLabels:['Page'], relationshipTypes:['LINKS']})`, `algo.bfs(source, -1, 'LINKS')`, `algo.betweenness({nodeLabels:['Page'], relationshipTypes:['LINKS'], samplingSize:3, samplingSeed:10})`, `algo.labelPropagation({nodeLabels:['Page'], relationshipTypes:['LINKS']})`, `algo.SPpaths({...})`, `algo.SSpaths({...})`, and `algo.MSF({nodeLabels:['Page'], relationshipTypes:['LINKS'], weightAttribute:'weight'})` with `YIELD nodes, edges`. Observed synthetic outputs: PageRank ids `a,b,c,d` with positive scores; WCC and label propagation grouped linked `a,b,c` separately from isolated `d`; BFS from `a` returned node/edge counts `[2, 2]`; deterministic betweenness on `a->b->c` returned center score `b=1.0`; SPpaths returned two paths with weights `[1.0, 2.0]`; SSpaths returned three paths with weights `[1.0, 1.0, 2.0]`; MSF returned forest node/edge counts `[[1, 0], [3, 2]]`. Treat this as synthetic output-shape proof only; production graph suitability and performance still need target-data benchmarks.
- Negative runtime contract smoke on the same image confirmed FalkorDB rejects Neo4j/GDS carryover shapes: MSF `YIELD edge, weight` rejected with `does not yield output \`edge\``; `CALL gds.pageRank.stream(...)` rejected with `Procedure \`gds.pageRank.stream\` is not registered`; `algo.pageRank(... ) YIELD nodeId` rejected with `does not yield output \`nodeId\``. Use FalkorDB procedure names and yields; do not translate Neo4j GDS examples mechanically.
