...
2. Python remains a behavioral reference until whole-system parity. A surviving Python harness may orchestrate processes, Cargo, parity, docs, CI, and GSD only.
...
7. Treat upstream Rust rules as advisory retrieval material. `references/project-policy.md` overrides generic guidance.
8. Before editing an existing symbol, follow project GitNexus impact-analysis requirements. Before completion, produce fresh verification evidence.
</essential_principles>
...
- `references/verification-matrix.md` — bounded checks by change type and failure surface.
