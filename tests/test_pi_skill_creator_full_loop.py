...
def test_execute_pi_skill_eval_command_backend_and_loop_integration(tmp_path: Path) -> None:
    ...
    assert all(
        (run_dir / "outputs" / "answer.md").is_file()
        for run_dir in iteration.glob("eval-*/*")
        if (run_dir / "outputs").is_dir()
    )


def test_executor_prompt_uses_run_local_absolute_outputs_and_preserves_file_artifact(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "workspace"
    result = run_cmd(
    ...
    assert result.returncode == 2, result.stderr + result.stdout
    iteration = workspace / "iteration-1"
    prompt = next(iteration.glob("eval-*/*/EXECUTOR_PROMPT.md")).read_text(encoding="utf-8")
    assert str(workspace.resolve()) in prompt
    assert "repository root `outputs/`" in prompt

    command = (
        f"{sys.executable} -c "
        '"import pathlib,re,sys; prompt=sys.argv[1]; '
        "path=pathlib.Path(re.search(r'`([^`]+/answer\\.md)`', prompt).group(1)); "
        "path.parent.mkdir(parents=True, exist_ok=True); "
        "path.write_text('artifact answer from command', encoding='utf-8'); "
        "print('stdout summary only')\""
    )
    result = run_cmd(
        [
            sys.executable,
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    run_json = next(iteration.glob("eval-*/with_skill/outputs/run.json"))
    status = json.loads(run_json.read_text(encoding="utf-8"))
    assert status["answer_source"] == "executor_created_file"
    assert (
        run_json.with_name("answer.md").read_text(encoding="utf-8")
        == "artifact answer from command"
    )
...
def test_package_pi_skill_excludes_pycache_and_quality_reports(tmp_path: Path) -> None:
    ...
    result = run_cmd(
        [sys.executable, str(PACKAGE), str(PI_SKILL), "--output", str(archive), "--require-evals"]
    )
    assert result.returncode == 0, result.stderr + result.stdout
    ...
    evals["evals"].append(
        {
            ...
            "files": [],
            "expectations": [
                "The pi skill creator should not trigger for ordinary React debugging."
            ],
        }
    )
