...
def run_quality(
    tmp_path: Path | None = None, skills_dir: Path = SKILLS_DIR, *extra: str
...
def test_pack_quality_passes_all_checks(tmp_path: Path) -> None:
    ...
    assert (
        (tmp_path / "pack-quality.md")
        .read_text(encoding="utf-8")
        .startswith("# FalkorDB Focused Skill Pack Quality Evaluation")
    )
...
def test_pack_quality_fails_when_focused_reference_is_thin(tmp_path: Path) -> None:
    ...
    assert report["summary"]["failed"] > 0
    assert any(
        item["skill"] == "falkordb-genai-mcp-graphrag" and not item["passed"]
...
def test_pack_quality_detects_project_specific_leakage(tmp_path: Path) -> None:
    ...
    report = json.loads((tmp_path / "pack-quality.json").read_text(encoding="utf-8"))
    assert any(
        "LegalGraph Nexus" in item["description"] and not item["passed"]
        for item in report["results"]
    )
