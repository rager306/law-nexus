...
_GOLDEN_CORPUS_MINIMAL_HEADER = (
    "schema_version: law-nexus-golden-corpus-catalog/v1\n"
    'lifecycle: "[proposed]"\n'
    "authoritative: false\n"
)
_GOLDEN_CORPUS_CLOSED_LISTS = (
    "tiers:\n"
    "  - G0\n"
    "  - G1\n"
    "  - G2\n"
...
_GOLDEN_CORPUS_ALLOWED_DEP_FILES = (
    "assertion-lifecycle-contract.yaml",
    "pending-effects-contract.yaml",
    "operation-registry.yaml",
    "reference-binding-contract.yaml",
    "evidence-anchor-contract.yaml",
    "merkle-roots-contract.yaml",
    "force-interval-set-contract.yaml",
    "scope-aware-completeness-contract.yaml",
)
_GOLDEN_CORPUS_GROUP_CYCLE = (
    "assertion-bitemporal",
    "pending-effects",
    "structure-identity",
    "source-cst",
...
def _write_golden_corpus_companion_deps(root: Path) -> None:
    architecture = root / "prd" / "architecture"
    architecture.mkdir(parents=True, exist_ok=True)
    for name in _GOLDEN_CORPUS_ALLOWED_DEP_FILES:
...
def _golden_corpus_case_block(
    index: int, *, group: str | None = None, extra_dep: str | None = None
) -> str:
    case_id = f"GC-{index:03d}"
    resolved_group = group if group is not None else _GOLDEN_CORPUS_GROUP_FOR_ID[index - 1]
    deps = ["      - prd/architecture/assertion-lifecycle-contract.yaml"]
    if extra_dep is not None:
        deps.append(f"      - {extra_dep}")
    return (
        "  -\n"
        f"    id: {case_id}\n"
        f"    review_anchor: {index}\n"
...
def _write_golden_corpus_catalog(root: Path, body: str) -> None:
    target = root / "prd" / "architecture" / "golden-corpus-catalog.yaml"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(body, encoding="utf-8")


def _valid_golden_corpus_catalog_text() -> str:
    cases = "".join(_golden_corpus_case_block(index) for index in range(1, 41))
    return _GOLDEN_CORPUS_MINIMAL_HEADER + _GOLDEN_CORPUS_CLOSED_LISTS + "cases:\n" + cases


def test_golden_corpus_catalog_passes_on_valid_fixture(tmp_path: Path) -> None:
    _write_golden_corpus_companion_deps(tmp_path)
    _write_golden_corpus_catalog(tmp_path, _valid_golden_corpus_catalog_text())
    ...
    assert "coverage=8/6/6/5/9/6" in finding.observed
    assert "lifecycle [bounded]; process anti-drift; not product readiness" in finding.observed


def test_golden_corpus_catalog_warns_when_missing(tmp_path: Path) -> None:
    findings = check_golden_corpus_catalog(tmp_path)
    assert len(findings) == 1
    finding = findings[0]
    assert finding.check_id == "golden-corpus-catalog"
    assert finding.status == "fail"
    assert finding.severity == "warn"
    assert finding.rule_id == "golden-corpus-catalog.missing"
    assert "present=false" in finding.observed


def test_golden_corpus_catalog_warns_on_lifecycle_not_proposed(tmp_path: Path) -> None:
    _write_golden_corpus_companion_deps(tmp_path)
    body = _valid_golden_corpus_catalog_text().replace(
        'lifecycle: "[proposed]"', 'lifecycle: "[bounded]"'
    )
    _write_golden_corpus_catalog(tmp_path, body)
    findings = check_golden_corpus_catalog(tmp_path)
    finding = findings[0]
    assert finding.status == "fail"
    assert finding.severity == "warn"
...
def test_golden_corpus_catalog_warns_on_duplicate_id(tmp_path: Path) -> None:
    _write_golden_corpus_companion_deps(tmp_path)
    body = _valid_golden_corpus_catalog_text().replace("id: GC-002", "id: GC-001", 1)
    _write_golden_corpus_catalog(tmp_path, body)
    findings = check_golden_corpus_catalog(tmp_path)
    finding = findings[0]
    assert finding.status == "fail"
    assert finding.severity == "warn"
    assert "ids-not-unique" in finding.observed or "ids-not-gc-001-040-in-order" in finding.observed


def test_golden_corpus_catalog_warns_on_coverage_drift(tmp_path: Path) -> None:
    _write_golden_corpus_companion_deps(tmp_path)
    body = _valid_golden_corpus_catalog_text().replace(
        "assertion-bitemporal: 8", "assertion-bitemporal: 7"
    )
    _write_golden_corpus_catalog(tmp_path, body)
    findings = check_golden_corpus_catalog(tmp_path)
    finding = findings[0]
    assert finding.status == "fail"
    assert finding.severity == "warn"
    assert "coverage=" in finding.observed


def test_golden_corpus_catalog_warns_on_missing_dep_file(tmp_path: Path) -> None:
    _write_golden_corpus_companion_deps(tmp_path)
    (tmp_path / "prd" / "architecture" / "assertion-lifecycle-contract.yaml").unlink()
    _write_golden_corpus_catalog(tmp_path, _valid_golden_corpus_catalog_text())
    findings = check_golden_corpus_catalog(tmp_path)
    finding = findings[0]
    assert finding.status == "fail"
    assert finding.severity == "warn"
    assert "dep-missing=" in finding.observed
