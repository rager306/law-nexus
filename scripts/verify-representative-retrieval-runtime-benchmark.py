...
def write_report(path: Path, payload: Mapping[str, Any]) -> None:
    report = "\n".join(
        [
            ...
            f"- Diagnostic codes: `{', '.join(payload['diagnostic_codes']) if payload['diagnostic_codes'] else 'none'}`",
            f"- Runtime boundary confirmed: `{str(payload['runtime_boundary_confirmed']).lower()}`",
            f"- Manifest corpus: `{payload['manifest'].get('corpus_id', 'unavailable')}`",
            f"- Query labels: `{payload['manifest'].get('query_label_count', 0)}`",
            f"- Candidate references: `{payload['manifest'].get('candidate_reference_count', 0)}`",
