...
EVAL_CHECKS: dict[int, list[Check]] = {
    1: [
        Check(
            "SKILL.md",
            ("Vector/full-text/index/procedure/UDF claims", "workflows/check-capability.md"),
            "vector/capability routing",
        ),
        Check(
            ...
            "complete claim classes",
        ),
        Check(
            "references/capability-evidence.md",
            ("Neo4j vector syntax", "APOC", "GDS", "Aura"),
            "Neo4j drift warning",
        ),
        Check(
            ...
            "separated vector proof layers",
        ),
        Check(
            "templates/capability-answer.md",
            ("Status:", "Evidence:", "Caveats:", "Next proof:", "Verification criterion:"),
            "capability answer template",
        ),
    ],
    2: [
        Check(
            "SKILL.md",
            ("Design/review graph schema", "workflows/design-graph-model.md"),
            "graph model routing",
        ),
        Check(
        ...
        Check(
            ...
            "direction/cardinality requirement",
        ),
        Check(
            "references/graph-modeling.md",
            ("query pattern it supports", "verification query"),
            "index access-pattern mapping",
        ),
        Check(
            "templates/graph-model-review.md",
            ("Query coverage", "Indexes", "Risks", "Verification plan"),
            "graph review template",
        ),
    ],
    3: [
        Check(
            "SKILL.md",
            ("Write, translate, or review FalkorDB Cypher", "workflows/write-cypher.md"),
            "Cypher routing",
        ),
        Check(
            "references/capability-evidence.md",
            (
                ...
                "Neo4j GenAI",
                "Neo4j driver",
                "Neo4j-specific Cypher",
            ),
            "Neo4j-only feature filter",
        ),
        Check(
            "references/cypher-falkordb.md",
            ("Parameterize values", "do not interpolate user input"),
            "parameterization guardrail",
        ),
        Check(
            "templates/query-review.md",
            ("Schema assumptions", "Expected result", "Verification fixture", "Safety review"),
            "query review template",
        ),
        Check(
            "workflows/write-cypher.md",
            ("procedures", "workflows/check-capability.md"),
            "non-core behavior proof gate",
        ),
    ],
    4: [
        Check(
            "SKILL.md", ("falkordb-py", "workflows/use-python-client.md"), "Python client routing"
        ),
        Check(
            "workflows/use-python-client.md", ("Identify runtime mode",), "runtime mode preflight"
        ),
        Check(
            "references/python-client.md",
            ("Do not log credentials", "outside source code"),
            "secret/config guardrail",
        ),
        Check(
            "workflows/use-python-client.md",
            ("parameter", "rather than formatting values into query strings"),
            "parameter passing guardrail",
        ),
        Check(
            "workflows/use-python-client.md",
            ("graph name", "operation", "phase", "duration", "error"),
            "observability requirement",
        ),
    ],
    5: [
        Check(
            "SKILL.md", ("FalkorDBLite", "workflows/use-falkordblite.md"), "FalkorDBLite routing"
        ),
        Check(
            "references/falkordblite.md",
            ("local testing", "quick demos", "CI-like smoke"),
            "embedded intended role",
        ),
        Check(
            "references/falkordblite.md",
            ("Do not assume server/container behavior", "Do not assume FalkorDBLite behavior"),
            "embedded/server boundary",
        ),
        Check(
            "workflows/use-falkordblite.md",
            ("startup", "basic query", "cleanup"),
            "embedded verification",
        ),
        Check(
            "workflows/use-falkordblite.md",
            ("indexes", "full-text", "vector", "UDF", "persistence", "concurrency", "resource"),
            "capability routing for embedded claims",
        ),
    ],
    6: [
        Check(
            "SKILL.md",
            ("Slow queries/errors/runtime failures", "workflows/debug-performance.md"),
            "debug routing",
        ),
        Check(
            "workflows/debug-performance.md",
            ("graph name", "operation", "query text", "cardinality", "runtime mode", "exact error"),
            "narrow reproduction",
        ),
        Check(
        ...
        Check(
            "workflows/debug-performance.md", ("smallest synthetic dataset",), "synthetic fixture"
        ),
        Check(
            "workflows/debug-performance.md",
            ("one variable at a time", "Rerun"),
            "one-change verification loop",
        ),
    ],
}
...
def write_markdown(report: dict, path: Path) -> None:
    ...
    for result in report["results"]:
        ...
        lines.extend(
            [f"### Eval {result['eval_id']} — {status}", "", f"Prompt: {result['prompt']}", ""]
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate FalkorDB skill quality against deterministic eval rubrics"
    )
    parser.add_argument("--root", type=Path, default=Path(".agents/skills/falkordb"))
    parser.add_argument(
        "--evals", type=Path, default=Path(".agents/skills/falkordb/evals/evals.json")
    )
    parser.add_argument(
        "--json-output",
        type=Path,
        default=Path(".agents/skills/falkordb/evals/quality-report.json"),
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        default=Path(".agents/skills/falkordb/evals/quality-report.md"),
    )
