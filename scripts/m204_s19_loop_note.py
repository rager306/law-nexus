#!/usr/bin/env python3
"""Closed, bounded census of the post-S18 validate abort and supervisor exit."""
...
SCHEMA = "law-nexus/gsd-post-s18-validate-abort/v1"
MANIFEST_SCHEMA = "law-nexus/m204-s19-frozen-hashes/v1"
DEFAULT_CENSUS = ROOT / "prd/migration/rust-evidence/m204-s19-post-s18-validate-abort.json"
DEFAULT_MANIFEST = ROOT / "prd/migration/rust-evidence/m204-s19-frozen-hashes.json"
PINNED = (
    (
...
def safe_file(root: Path, value: Any) -> Path:
    ...
    if (
        ...
        or "\\" in value
        or ".." in path.parts
        or any(part in {".git", ".gsd", ".planning", ".audits"} for part in path.parts)
    ):
...
def aborts() -> list[dict[str, Any]]:
    return [
        {
            "ordinal": 1,
            "unit_start_at": "2026-09-12T19:44:25.735Z",
            "unit_end_at": "2026-09-12T19:58:24.062Z",
            "unit_end_status": "no-artifact",
            "finalize_status": "retry",
            "iteration_end_reason": ABORT_MESSAGE,
            "flow_id": "38a570c5-47c3-4fcb-af6f-45372aadcf26",
            "unit_id": "M204-w2ktfw",
...
def supervisor_exit() -> dict[str, Any]:
    return {
        ...
        "query_at": "2026-09-12T19:44:07Z",
        "classify_at": "2026-09-12T19:58:33Z",
        "classify_status": "blocked",
        "interrupted": False,
        "tool_calls": 62,
        "journal_unit_start_present": True,
        "headless_pid": "1175128",
        "unit_id": "M204-w2ktfw",
        "unit_type": "validate-milestone",
        "stop_at": "2026-09-12T19:58:36Z",
        ...
        "next_unit_id": "M204-w2ktfw/S19",
        "next_unit_start_at": "2026-09-12T20:04:18.077Z",
    }
...
def compose(root: Path, census: Path, manifest: Path) -> None:
    ...
    document = {
        ...
        "slice": "S19",
        "predecessor_slice": "S18",
        "s18_complete_at": "2026-09-12T19:42:49.393Z",
        "s18_uat_end_at": "2026-09-12T19:44:06.249Z",
        "abort_count": 1,
        "aborts": aborts(),
        ...
        "frozen_manifest": {"schema": MANIFEST_SCHEMA, "path": relative(root, manifest)},
    }
    manifest_doc = {
        "schema": MANIFEST_SCHEMA,
        "scope": "S19 predecessor-only; twelve frozen tracked snapshots; no self-hash and no corpus walk",
        "files": frozen,
        "non_claims": [
            "census records do not repair the validate engine",
            "census does not call validate-milestone or change lifecycle state",
            "supervisor_exit is separate from the one no-artifact abort",
...
def check(root: Path, census: Path, manifest: Path) -> None:
    ...
    if (
        doc["s18_complete_at"] != "2026-09-12T19:42:49.393Z"
        or doc["s18_uat_end_at"] != "2026-09-12T19:44:06.249Z"
    ):
        raise ValueError("predecessor timestamps mismatch")
    if type(doc["abort_count"]) is not int or doc["abort_count"] != 1 or doc["aborts"] != aborts():
    ...
    if doc["frozen_manifest"] != {"schema": MANIFEST_SCHEMA, "path": relative(root, manifest)}:
        raise ValueError("frozen manifest reference mismatch")
    print("S19_T01_CENSUS_OK")
...
def main() -> int:
    ...
    try:
        operation = compose if args.command == "compose" else check
        operation(args.root.resolve(), args.census, args.manifest)
        return 0
    except (OSError, ValueError, KeyError, TypeError, json.JSONDecodeError) as exc:
        print(f"m204_s19_loop_note: {exc}", file=sys.stderr)
        return 1
