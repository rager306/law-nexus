#!/usr/bin/env python3
"""Closed, bounded census of the post-S19 validate loop."""
...
SCHEMA = "law-nexus/gsd-post-s19-validate-loop/v1"
MANIFEST_SCHEMA = "law-nexus/m204-s20-frozen-hashes/v1"
DEFAULT_CENSUS = ROOT / "prd/migration/rust-evidence/m204-s20-post-s19-validate-loop.json"
DEFAULT_MANIFEST = ROOT / "prd/migration/rust-evidence/m204-s20-frozen-hashes.json"
PINNED = (
    (
...
def aborts() -> list[dict[str, Any]]:
    rows = [
        (
            1,
            "2026-09-12T20:50:47.453Z",
            "2026-09-12T21:00:38.803Z",
            "041c0f6f-bdee-4e84-91d2-b8299cde3d0c",
        ),
        (
            2,
            "2026-09-12T21:02:15.992Z",
            "2026-09-12T21:14:34.843Z",
            "ffe37bd6-6432-4f70-a1eb-f35f376cf47e",
        ),
        (
            3,
            "2026-09-12T21:16:08.902Z",
            "2026-09-12T21:27:41.606Z",
            "e82c0df7-693b-4110-8fe8-e52cafc21c83",
        ),
        (
            4,
            "2026-09-12T21:29:21.750Z",
            "2026-09-12T21:42:37.636Z",
...
def supervisor_exit() -> dict[str, Any]:
    return {
        ...
        "query_at": "2026-09-12T21:28:24Z",
        "classify_at": "2026-09-12T21:43:05Z",
        "classify_status": "blocked",
        "interrupted": False,
        "tool_calls": 7,
        "journal_unit_start_present": True,
        "headless_pid": "1332614",
        "unit_id": "M204-w2ktfw",
        "unit_type": "validate-milestone",
        "stop_at": "2026-09-12T21:43:07Z",
        ...
        "next_unit_id": "M204-w2ktfw/S20",
        "next_unit_start_at": "2026-09-13T01:43:20.508Z",
    }
...
def compose(root: Path, census: Path, manifest: Path) -> None:
    ...
    document = {
        ...
        "slice": "S20",
        "predecessor_slice": "S19",
        "s19_complete_at": "2026-09-12T20:48:11.249Z",
        "s19_uat_end_at": "2026-09-12T20:50:28.370Z",
        "abort_count": 4,
        "aborts": aborts(),
        ...
        "s20_called_validate_milestone": False,
    ...
    manifest_doc = {
        ...
        "scope": "S20 predecessor-only; fourteen frozen tracked snapshots; no self-hash and no corpus walk",
        "files": frozen,
        "non_claims": [
            "census records do not repair the validate engine",
            "census does not call validate-milestone or change lifecycle state",
            "supervisor_exit is separate from the four no-artifact aborts",
...
def check(root: Path, census: Path, manifest: Path) -> None:
    ...
    if (doc["s19_complete_at"], doc["s19_uat_end_at"]) != (
        "2026-09-12T20:48:11.249Z",
        "2026-09-12T20:50:28.370Z",
    ):
        raise ValueError("predecessor timestamps mismatch")
    if type(doc["abort_count"]) is not int or doc["abort_count"] != 4 or doc["aborts"] != aborts():
...
def main() -> int:
    ...
    try:
        ...
        print(f"m204_s20_loop_note: {exc}", file=sys.stderr)
        return 1
