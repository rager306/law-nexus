...
RecordKind = Literal["document", "source_block", "consultant_hierarchy", "relation_candidate"]
...
EXPECTED_SCHEMA_FILES = {
    ...
    "consultant_hierarchy_record": SCHEMA_DIR / "consultant_hierarchy_record.schema.json",
...
def schema_artifacts() -> list[GeneratedArtifact]:
    ...
    schema_inputs: list[tuple[str, Path, dict[str, Any]]] = [
        ...
        (
            "ConsultantHierarchyRecord",
            EXPECTED_SCHEMA_FILES["consultant_hierarchy_record"],
            ConsultantHierarchyRecord.model_json_schema(),
        ),
        (
            "RelationCandidateRecord",
...
This contract defines the strict parser boundary records for downstream M006 parser and graph staging work. `DocumentRecord`, `SourceBlockRecord`, `ConsultantHierarchyRecord`, and `RelationCandidateRecord` are provenance-preserving Pydantic v2 models with generated JSON Schema artifacts and JSONL examples.

The records are non-authoritative inspection artifacts. They validate shape, provenance, bounded excerpts, candidate status, and non-claim preservation; they do not validate legal correctness, parser completeness, product ETL readiness, or FalkorDB loading/runtime readiness.
...
| `prd/parser/schemas/consultant_hierarchy_record.schema.json` | JSON Schema for deterministic Consultant WordML hierarchy/source records. |
...
| `consultant_hierarchy` | `ConsultantHierarchyRecord` | Consultant WordML hierarchy/source records preserve stable parent IDs, level enum, marker metadata, and bounded excerpt hashes without legal-authority claims. |
...
- This contract does not claim FalkorDB loading/runtime readiness.
- This contract preserves Consultant WordML legal-authority non-claims; Consultant WordML XML remains relation fixture / prior-art evidence until later validation proves candidate relations.
...
- M009 Consultant WordML hierarchy parsing should emit `consultant_hierarchy` records only after deterministic source-shape extraction proves level, parent, marker, and excerpt-hash evidence for each accepted paragraph.
...
def build_summary(
    *,
    ...
    ordered_counts = {
        ...
        "consultant_hierarchy": counts.get("consultant_hierarchy", 0),
