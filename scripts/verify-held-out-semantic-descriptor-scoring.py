...
def run_json_command(command: Sequence[str], timeout_seconds: int) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        raise HeldOutDescriptorScoringError(
...
def compute_metrics(
    ...
    for case_id, expected in expected_by_case.items():
        ...
        if not ranked_ids:
            raise HeldOutDescriptorScoringError(
                f"observed scores missing for held-out case: {case_id}"
            )
...
def build_report(
    ...
    *,
    ...
    diagnostics.add(
        "held_out_descriptor_scoring_completed"
        if scoring_status == "completed"
        else "held_out_descriptor_scoring_blocked"
    )
    report = {
