...
PROOF_FIELD_CHECKS: tuple[tuple[tuple[str, ...], Any, str], ...] = (
    (
        ("disposable_repo", "is_inside_main_law_nexus"),
        False,
        "disposable repo is outside the main law-nexus checkout",
    ),
    (("stages", "init", "exit_code"), 0, "git lex init exit code"),
    (
        ("stages", "init", "auto_commit_landed"),
        True,
        "init auto-commit landed (MEM549 inversion: hook found git-lex on cold PATH)",
    ),
    (("stages", "init", "hook_installed"), True, "init installed the pre-commit hook"),
    (("stages", "init", "lex_dir_created"), True, "init created the .lex directory"),
    (("stages", "content_seed_commit", "exit_code"), 0, "content seed plain git commit exit code"),
    (
        ("stages", "content_seed_commit", "hook_extracted_spo"),
        True,
        "content seed commit triggered hook extract (.spo sidecar)",
    ),
    (("stages", "sync", "exit_code"), 0, "git lex sync exit code"),
    (("stages", "query", "exit_code"), 0, "git lex query exit code"),
    (("stages", "validate", "exit_code"), 0, "git lex validate exit code"),
    (("stages", "list", "exit_code"), 0, "git lex list exit code"),
    (("residue_guard", "before", "Squad"), "absent", "residue_guard.before Squad absent"),
    (("residue_guard", "before", "Raw"), "absent", "residue_guard.before Raw absent"),
    (("residue_guard", "before", ".artifacts"), "absent", "residue_guard.before .artifacts absent"),
    (("residue_guard", "after", "Squad"), "absent", "residue_guard.after Squad absent"),
    (("residue_guard", "after", "Raw"), "absent", "residue_guard.after Raw absent"),
    (("residue_guard", "after", ".artifacts"), "absent", "residue_guard.after .artifacts absent"),
    (
        ("residue_guard", "r047_contract_phase"),
        "honored",
...
def check_proof_file(proof: Path) -> list[Diagnostic]:
    ...
    if data is None:
        diagnostics.append(
            _diagnostic(
                "missing_proof_file", proof, 0, f"workflow-proof is not valid JSON: {proof}"
            )
        )
        return diagnostics
    return diagnostics
