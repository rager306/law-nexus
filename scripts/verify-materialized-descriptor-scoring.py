#!/usr/bin/env python3
"""Run M027 local semantic scoring over materialized-derived safe descriptors."""
...
class MaterializedDescriptorScoringError(RuntimeError):
    """Raised when materialized descriptor scoring cannot safely produce a proof."""
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise MaterializedDescriptorScoringError(f"input_missing: {path}") from exc
    except json.JSONDecodeError as exc:
        raise MaterializedDescriptorScoringError(f"malformed JSON: {path}: {exc.msg}") from exc
    if not isinstance(payload, dict):
        raise MaterializedDescriptorScoringError(f"JSON root must be object: {path}")
    return payload
...
def assert_safe_payload(payload: Mapping[str, Any], *, allow_labels: bool = False) -> None:
        ...
        if isinstance(value, Mapping):
            for key, child in value.items():
                if str(key) in forbidden:
                    raise MaterializedDescriptorScoringError(f"unsafe field name: {key}")
                walk(child)
    ...
    for fragment in FORBIDDEN_STRING_FRAGMENTS:
        ...
        if fragment.lower() in serialized.lower():
            raise MaterializedDescriptorScoringError(f"unsafe payload fragment: {fragment}")
    if "provider_payloads_excluded" in serialized:
        raise MaterializedDescriptorScoringError("unsafe durable vocabulary")
...
def scores_from_json(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(scores, list):
        raise MaterializedDescriptorScoringError("observed scores missing")
...
def load_evaluation_labels(path: Path = EVALUATION_LABELS) -> dict[str, set[str]]:
    ...
    if not isinstance(labels, list) or not labels:
        raise MaterializedDescriptorScoringError("evaluation labels missing")
    ...
    for row in labels:
        ...
        if not isinstance(expected, list) or not expected:
            raise MaterializedDescriptorScoringError("expected candidates missing")
    ...
    if payload.get("label_count") != len(expected_by_case):
        raise MaterializedDescriptorScoringError("evaluation label count mismatch")
...
def build_report(
    ...
    *,
    ...
    report = {
        ...
        "scoring_mode": SCORING_MODE,
        "descriptor_inputs_artifact": "prd/research/ontology_architecture_requirements/fixtures/materialized_descriptor_inputs.json",
        ...
        "m026_held_out_result_metrics": M026_HELD_OUT_RESULT,
        "metrics": metrics,
        "metric_deltas_vs_m024": metric_deltas(metrics, M024_BASELINE, "delta_vs_m024"),
        "metric_deltas_vs_m026": metric_deltas(metrics, M026_HELD_OUT_RESULT, "delta_vs_m026"),
        ...
        json.dumps(
            {
                ...
                "metric_deltas_vs_m026": report["metric_deltas_vs_m026"],
