...
REPORT_MD = "consultant_relation_candidates.md"
...
def render_markdown(report: dict[str, Any]) -> str:
    """Render a bounded human-readable Consultant relation candidate report."""
    ...
    for candidate in report["candidates"]:
        lines.append(
            "| {id} | `{selector}` | `{status}` | `{object_ref}` | `{base}` | `{n}` | `{date}` | `{demo}` |".format(
                id=candidate["id"],
