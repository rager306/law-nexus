...
| `EvidenceSpan` | future source-bound byte-span evidence contract referenced by readiness prose | ADR-0010 + prd/architecture/evidence-anchor-contract.yaml | canonical design term `[proposed]` | wording does not assert a current public type, real-corpus coverage or legal truth; alias of EvidenceAnchor/EvidenceBundle, not a third entity / not ln-decode::EvidenceAnchor |
...
| NormativeState | time-indexed normative status with six written statuses (`InForce`, `NotYetInForce`, `Suspended`, `Repealed`, `Expired`, `Invalidated`) per the force-interval YAML (D251 / ADR-0018 G0(c) additive note); `Superseded` is a VersionRelation (Replaces / Supersedes / Corrects), not a written force status; `Unknown` is the fail-closed point-query outcome, not a written status; `Transitional` is the F13-T fork moved to the ADR-0021 overlay, not a force state; interval algebra per `prd/architecture/force-interval-set-contract.yaml`; the bounded runtime 6-set (kb-ontology.yaml `force_status_values`) is not this list (a cardinality match is not a member match); force orthogonal to version/applicability/epistemic (RC11-F09 design inventory) | ADR-0018 | canonical `[proposed]` | text presence does not imply `InForce`; InForce ≠ Applicable |
...
| CC-path | recursive structural locator (`cc:…:statya-93/punkt-4/punkt-4.2`) generalizing flat registry bindings; repairs the changed_article_texts bare-number key defect; prepares the TQ-05 reference channel; living alias name `ComponentLocator` (review-26 §4): living alias of the recursive structural locator slot historically named CC-path, not a second first-cell and not identity canon | ADR-0016 amendments (R8-11); review-26 §4 living overlay | canonical design term `[proposed]` | not a new identity canon; Work identity stays number + authority + date as the historical D216 wording beside the `OfficialIdentityClaim` natural-key rule (review-26 §4); not minted by parsed_as binding |
| `LegalEventAssertion` | append-only bitemporal ledger record of an asserted legal event: evidence span, `recorded_at`, `asserted_by`, status (Proposed / Validated / AuthoritativeInternal / Rejected / Superseded); that five-set is the projection-status surface, not the living adjudication vocabulary; the ledger is the canonical history of a Work; the living adjudicative entities `ValidationReceipt` / `AssertionDispositionEvent` / `AssertionRelation` live in `prd/architecture/assertion-lifecycle-contract.yaml` | ADR-0017 G0(a) amendment (D216) + review-26 P0-3 living overlay | canonical design term `[proposed]` | candidate ≠ fact; correction appends a new assertion and rebuilds projections, never rewrites the known_as_of past; `AcceptedForProjection` is the living alias of the projection-status slot `AuthoritativeInternal`, not a sixth status and not a rename of the five-set; `CanonicalInternal` is not a living name; `AssertionRelation.Supersedes` is not `VersionRelation.Supersedes` |
| `ValidationReceipt` | one validation act over an assertion candidate (review-26 P0-3 / D255): who validated what, when, with which result and which validator build; fields `assertion_id`, `validation_kind`, `result`, `validator`, `validated_at`, `validator_version`; definition data in `prd/architecture/assertion-lifecycle-contract.yaml` | ADR-0017 G0(a) | canonical design term `[proposed]` | design data, never a runtime object; `validation_kind` and `result` are field names, not closed enums; the four typed receipts of review-26 §8.4 are not minted (no `TechnicalValidationReceipt` or siblings) |
| `AssertionDispositionEvent` | one adjudication decision over an assertion (review-26 P0-3): carries exactly one living disposition (`AcceptedForProjection` / `Quarantined` / `Rejected` / `Retired`) plus `decided_at` / `decided_by` / `policy_version` / `rationale`; appends adjudication history beside the record instead of mutating a committed status; definition data in `prd/architecture/assertion-lifecycle-contract.yaml` | ADR-0017 G0(a) | canonical design term `[proposed]` | the disposition 4-set is not a sixth projection status; historical status `Rejected` and living disposition `Rejected` share a spelling and stay distinct closed lists; `Quarantined` is not ln-dispose Review Case quarantine; `Retired` is not ADR-status Retired; no entity `DispositionEvent` is minted |
| `AssertionRelation` | one directed assertion-to-assertion relation (review-26 P0-3): fields `from_assertion` / `to_assertion` / `kind`, with kind ∈ (`Supports` / `Contradicts` / `Corrects` / `Supersedes` / `Duplicates` / `Qualifies`); definition data in `prd/architecture/assertion-lifecycle-contract.yaml` | ADR-0017 G0(a) | canonical design term `[proposed]` | kind `Supersedes` is not `VersionRelation.Supersedes` (force-interval scalar) and kind `Corrects` is not `VersionRelation.Corrects`; historical status `Superseded` is not this relation kind; not cross-version identity |
| `WorkId` | opaque persistent identifier of a Work (act-level identity symmetric with `ComponentId`, review-26 §4); natural-key fields live on `OfficialIdentityClaim`, not on the identifier itself | ADR-0016 / review-26 §4 living overlay | canonical design term `[proposed]` | not the composed runtime `work:ru:…` mint string; not an ELI URN; not number + authority + date as internal id |
| `OfficialIdentityClaim` | authority, enactment_date, number, act_type, jurisdiction, source, recorded_at carried as the natural key for Work reconciliation, not the internal identifier | ADR-0016 / review-26 §4 living overlay | canonical design term `[proposed]` | `recorded_at` here is claim observation time, not `LegalEventAssertion.recorded_at`; the claim reconciles Works and is not itself the persistent identity of one |
...
| `DocumentaryPresence` | Tombstone / Present / Absent in the document structure — a separate repeal axis; living plane name `EditorialPresence` (review-26 §3): living alias of the documentary/editorial-presence slot historically named `DocumentaryPresence`, not a second first-cell and not a merkle root kind | ADR-0017 G0(f) amendment (D216); review-26 §3 living overlay | canonical design term `[proposed]` | never force by text; text presence ≠ status; cascade is derived `RepealScope` |
...
| `EffectSelector` | the selector vocabulary carried by a LegislativeEffect: living ActivationTrigger names are the operation-registry YAML 6-set after review-26 P0-4 (`At`, `AfterPublication`, `OnEvent`, `OnCondition`, `RetroactiveTo`, `Unknown`; D252 / ADR-0017 G0(c) additive note); `ForRelationsAfter` is not a selector mode — it is a TransitionPredicate (`RelationsArisingOnOrAfter`) on the AXIS-6 / TransitionConstraint plane; `OnCondition` is a guard, not a clock; an unproven `OnCondition` yields `TriggerUnknown`, never false | ADR-0017 G0(c) + ADR-0009 G0 note (D216); review-26 P0-4 / D252 | canonical design term `[proposed]` | not a sixth clock; `Unknown` never silently orders; non-commuting effects yield `OrderingConflict` |
...
| WALK-T | temporal walk of one Work Manifestation/Expression chain; `AST(t_k) = fold(log, t_k)`; the recursive history contour of Review 9 | ADR-0017 + Review 9 | canonical design term `[proposed]` | not the XML DOM of one edition; not WALK-S marker nesting; named-pair replay is not a 118-step chain |
| WALK-S | structural walk of one snapshot by rank `(role_order, depth)` producing membership drafts | ADR-0013 + ADR-0017 + Review 8/9 | canonical design term `[proposed]` | not force; not Applicable; not identity mint |
| WALK-I | identity walk: first appearance of a marker key in the Work chain mints a stable CC; reuse of a number on a different path is a new CC or Conflict | ADR-0016 + Review 9 | canonical design term `[proposed]` | not the latest-edition registry snapshot as the CC universe; edition-0001 is earliest oracle, not proven C0 |
| CST | lossless concrete source tree (crystal green projection) folded from the assertion ledger; informal session label CSM is this CST+AST compiler contour, not a third tree | ADR-0017 G0(a)/G0(d) | canonical design term `[proposed]` | not `StructuralAst`; not EditionSnapshot; not parser `SourceBlockRecord`; no runtime type |
| `StructuralAst` | fold projection of versioned membership events at effect_day t (`fold_membership_at`); Rust domain homonym of this projection type (`ln-temporal`) | ADR-0017 + KBO-R022 | canonical `[bounded]` runtime projection | not CST; not CTV text; not Expression bind; not force; not a second type |
| SHACL/SPARQL | RDF shape constraints and graph query as a possible later D046 L6 compatibility export after WALK-I and force-set alignment | D046 + R035 + R066 | anti-runtime `[proposed]`; archive-only RDF prior art | must not become the product ontology control plane, fold, or checkout; open-world absence is not fail-closed `Unknown` |
...
1. Work identity requires authority, enactment date and number; number alone is insufficient. This triple is the `OfficialIdentityClaim` natural-key rule (review-26 §4): the claim reconciles Works, while persistent internal identity remains the opaque `WorkId` design term.
...
`DocumentaryPresence` (living alias `EditorialPresence`, review-26 §3),
`OrderedMembershipVersion`, `EffectSelector`) is
