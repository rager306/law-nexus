...
| NormativeState | time-indexed normative status with seven written statuses (`InForce`, `NotYetInForce`, `Suspended`, `Repealed`, `Superseded`, `Expired`, `Invalidated`) per ADR-0018 G0(c); `Unknown` is the fail-closed point-query outcome, not a written status; `Transitional` is the F13-T fork moved to the ADR-0021 overlay, not a force state; interval algebra per `prd/architecture/force-interval-set-contract.yaml`; the bounded runtime 6-set (kb-ontology.yaml `force_status_values`) is not this list; force orthogonal to version/applicability/epistemic (RC11-F09 design inventory) | ADR-0018 | canonical `[proposed]` | text presence does not imply `InForce`; InForce ≠ Applicable |
