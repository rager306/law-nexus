...
- The punkt/subunit text-CTV layer notes in §5/§6 (KBO-R067 references, the
  executed `subordinates` CLI surface, the statya-only `inspect` gap) were
  appended by M172-tsa1j7 S01 T03; they add no lifecycle promotion.
...
| `government_resolution` | punkt; punkt(unit, recursive max_depth 3, suffix `.`)→podpunkt(subunit)→prilozhenie(container) | punkt unit collection executed on the real Garant ПП corpus (M171 S03): PP_60 primary + bounded breadth via the `subordinates` CLI report; nested sub-markers accumulate into the unit body; punkt-unit text-CTV is layer 2 of the KBO-R067 three-layer contract (mint level = the group's YAML `granularity: punkt`; wrong-level mint fails closed) `[bounded]` | `crates/ln-product-cli/tests/real_subordinate_acts.rs` | CCs are fixture-minted, not registry identity; no edition-day registry for ПП; amendment PPs (lettered а/б) honestly yield 0 punkt units; one embedded-image ODT fails decode; depth-ranked recursive walk remains `[proposed]` (KBO-R063); M172 S02 wired the PP_60 inspect surface, so inspect wires YAML granularity → on a bound ПП `government_resolution` it mints `punkt` and reports `ctv_resolved > 0` at that level (honest 0 only when the path is truly unbound, e.g. lettered amendment ПП); no ADR-0017 promotion |
| `departmental_order` | punkt; punkt(unit, recursive max_depth 4, suffix `)`)→podpunkt(subunit)→primechanie(subunit-text)→prilozhenie(container) | executed on an inline fixture only (M171 S03): two punkt units resolve text-CTV via the same `subordinates` CLI report (KBO-R067 layer 2) `[bounded]` | `crates/ln-product-cli/tests/real_subordinate_acts.rs` (`order_punkt_units_resolve_via_inline_fixture`) | no real приказ in the corpus — fixture-only proof, not corpus evidence |
...
| TextAst | resolve_CTV(cc, t) | `ln_kb_ontology::domain::resolve_ctv` over a `TextVersionLog` built by `build_text_log_from_markers` / `build_text_log_from_articles` | `crates/ln-kb-ontology/tests/resolve_ctv.rs`; `crates/ln-product-cli/tests/cli_contract.rs` (tracked 402-FZ fixture, `ctv_resolved > 0`); `crates/ln-product-cli/tests/real_44fz_text_ctv.rs` (consru_export [smoke]: ≥85 statya non-empty text, edition-0118) | text-facet drafts are observations, not membership records (KBO-R061); the real-corpus proof is skip-capable; punkt/subunit layering is the KBO-R067 three-layer contract (M172 S01) — statya mint on law groups, punkt-as-unit mint on ПП/приказ groups, punkt-as-subunit folded into the owning statya body, never one merged «punkt CTV» claim `[bounded]` |
