...
| `model-crystal-anchors` | Verify the Reviews 10–14 model crystal projection grounding: declared source sha256 digest, verbatim `anchor:` quotes against the immutable L0 review-25 file, required Layer 0/1 sections and INV-01..08, 09a/09b/09c, 10, 11, INV-22 definition rows (14 rows; no bare 09) | advisory `warn`; grounding drift only, never meaning or canon |
| `kb-ontology-draft` | Structural L1–L3 inventory: YAML FSM/vocabulary vs projection contract; `assembly_fsm.current` must stay off readiness `fsm.current`/states when present; optional honesty overlay vs `temporal-ast-review-control.yaml` compares runtime `force_status_values` **members** (not length) to living `written_statuses` | advisory `warn`; not production schema; D272 no new check_id |
...
| `semantic-name-collision` | Surface one PascalCase token defined by two unqualified owners/senses across glossary first cells, contract YAML entities and Rust domain types (review-26 P0-1 class); a pinned homonym qualifier such as EvidenceAnchor YAML vs ln-decode passes, and it is not the §11/P1-8 full semantic governor | heuristic advisory `warn` |
| `companion-manifest` | Every companion YAML backtick-mentioned in `prd/architecture/model-crystal.md` must exist and carry `schema_version`, `lifecycle [proposed]` and `authoritative: false` (review-26 §11.5 fail-closed injection support as a process net; not §11/P1-8, not a digest check); the expected set is extracted from crystal mentions, never a hardcoded second canon, and unmentioned YAML, review prose and archives stay out of surface | deterministic advisory `warn` |
| `golden-corpus-catalog` | The review-26 section 10 catalog `prd/architecture/golden-corpus-catalog.yaml` exists with `schema_version` `law-nexus-golden-corpus-catalog/v1`, `lifecycle [proposed]`, `authoritative: false`, closed tiers G0-G3, unique GC-001..GC-040 ids, group coverage 8/6/6/5/9/6 and deps that name existing companion YAML; process anti-drift, not case execution and not product readiness | deterministic advisory `warn` |
...
16. An unqualified dual-owner PascalCase token across glossary/YAML/Rust definition sites → heuristic advisory warn; a documented homonym qualifier → pass; default exit stays 0.
17. A crystal-mentioned companion YAML that is absent or lacks `schema_version` / `lifecycle [proposed]` / `authoritative: false` → deterministic advisory warn; unmentioned YAML and review prose are ignored; default exit stays 0.
18. A golden-corpus catalog that is absent, lacks the three design-only headers, drifts off the closed four-tier set / unique GC-001..GC-040 / group coverage 8/6/6/5/9/6, or names a missing companion dep → deterministic advisory warn; default exit stays 0.
