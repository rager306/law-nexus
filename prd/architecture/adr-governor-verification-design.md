...
- exact repository-relative line evidence for ADR lifecycle, link, supersession, index lifecycle, ontology weave, structure, cross-surface citation, forward-roadmap sequence, roadmap freshness state/JSON anchors, published PC/RQ rows, semantic-stub, retired-ID and era-noise findings when a concrete source line exists; missing surfaces or absent expected rows remain honest path-only evidence;
