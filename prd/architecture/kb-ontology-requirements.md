...
| **2. punkt-as-unit text-CTV** (M171; anchor PP_60) | one `ArticleText` per `punkt` unit (`government_resolution`, `departmental_order`); heading excluded; podpunkt folds into the owning punkt body; primechanie excluded (`departmental_order`) | `punkt` (YAML granularity); fixture-CC `cc:<act>:punkt-N`; day from filename ISO ordinal with provenance `fixture:subordinate:…` (edition-day registry parses `law_*` only) | `subordinates` CLI | not registry identity; not `inspect` (inspect hardcodes `"statya"` mint → honest 0 on PP; named gap, wiring is S02); not a Work mint |
