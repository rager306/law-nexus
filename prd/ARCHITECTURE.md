...
L7 ADR-0022 [proposed]  industry profiles                бюджет/стройка/медицина/общий = adapters
   ▼
   ADR-0023 [proposed]  applicability ownership boundary neutral core protocol + versioned profile inputs; `ln-applicability` v0 is fail-closed abstention-only with design-only NormRule IR (no Applicable/NotApplicable product claim)
```
