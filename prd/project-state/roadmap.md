...
- **Local GSD workflow marker:** M166-iyy4ak is the latest completed milestone in GSD registry after process reconcile of M161–M166. Product/design current-front remains evidence-gated; GSD completion is not product readiness.
- **GSD active dual-truth (DT-lag):** `M167-odlgt8` remains active in local GSD with pending slices while NormRule IR product/design evidence and RC11-F04a L_review closeout already exist (`1403294`, ceiling `spine`). Bridge register: `prd/architecture/review-cases/gsd-review-bridge.md`. Not GSD complete; not product readiness; not Applicable.
