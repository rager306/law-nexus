...
_GOLDEN_CORPUS_SCHEMA = "law-nexus-golden-corpus-catalog/v1"
_GOLDEN_CORPUS_TIERS = ("G0", "G1", "G2", "G3")
_GOLDEN_CORPUS_GROUPS = (
...
_GOLDEN_CORPUS_CASE_COUNT = 40


def _golden_corpus_top_scalars(text: str) -> dict[str, tuple[str, int | None]]:
    """Parser-free top-level ``key: value`` scan (companion-manifest twin)."""
    scalars: dict[str, tuple[str, int | None]] = {}
    for line_number, line in enumerate(text.splitlines(), start=1):
        top_key = _COMPANION_TOP_KEY_RE.match(line)
        if not top_key:
...
def _golden_corpus_dash_items(text: str, section: str, until: str) -> list[str]:
    """Items of a top-level dash list between ``section:`` and ``until:``."""
    ...
    for index, line in enumerate(lines):
        if line == f"{section}:":
            start = index + 1
            continue
        if start is not None and line and not line.startswith((" ", "\t")) and line.endswith(":"):
            if line.startswith(f"{until}:"):
                end = index
                break
    if start is None:
        return []
...
def _golden_corpus_coverage_map(text: str) -> dict[str, int]:
    """``coverage:`` mapping rows until the next top-level key."""
    rows: dict[str, int] = {}
    in_section = False
...
def _golden_corpus_allowed_deps(text: str) -> set[str]:
    return set(_golden_corpus_dash_items(text, "allowed_deps", "cases"))


def _golden_corpus_cases(text: str) -> list[dict[str, Any]]:
    """Line-shape case blocks after the top-level ``cases:`` key."""
    lines = text.splitlines()
    start = None
    for index, line in enumerate(lines):
        if line == "cases:":
    ...
    for line_number, line in enumerate(lines[start:], start=start + 1):
        if line.strip() == "-":
            ...
            current = {"_line": line_number, "deps": []}
            in_deps = False
            continue
        if current is None:
            continue
        field = re.match(r"^\s{4}(id|review_anchor|group|tier|status):\s*(.+)\s*$", line)
        if field:
            current[field.group(1)] = field.group(2).strip().strip("\"'")
            in_deps = False
            continue
        if re.match(r"^\s{4}deps:\s*$", line):
            in_deps = True
            continue
        dep = re.match(r"^\s{6}-\s+(.+)\s*$", line)
        if in_deps and dep:
            current["deps"].append(dep.group(1).strip().strip("\"'"))
    if current is not None:
        cases.append(current)
    return cases
...
def check_golden_corpus_catalog(root: Path) -> list[GovernorFinding]:
    """Advisory integrity net over the review-26 §10 golden-corpus catalog.

    Parser-free line-shape scan of ``prd/architecture/golden-corpus-catalog.yaml``:
    the three design-only headers (schema_version, lifecycle [proposed],
    authoritative false), closed four-tier set G0–G3, unique GC-### ids,
    coverage counters 8/6/6/5/9/6 summing to 40, and deps that name only
    existing companion YAML under ``prd/architecture/``. Lifecycle [bounded];
    process anti-drift, not case execution and not product readiness. This
    check does not mint Rust types, does not load YAML, and does not decide
    which catalog is canonical.
    """
    check_id = "golden-corpus-catalog"
    expected = (
        "The golden-corpus catalog exists with schema_version "
        f"{_GOLDEN_CORPUS_SCHEMA}, lifecycle [proposed], authoritative false, "
        "closed tiers G0-G3, unique GC-001..GC-040 ids, group coverage "
        "8/6/6/5/9/6 and deps that name existing companion YAML."
    )
    remediation = (
        "Restore prd/architecture/golden-corpus-catalog.yaml headers, the "
        "closed four-tier set, unique GC-### ids covering review-26 section 10 "
        "(40 cases, group counts 8/6/6/5/9/6), and deps that point at existing "
        "prd/architecture companion YAML. Do not invent cases, do not execute "
        "them, and do not mint Rust types from this check."
    )
    ...
    if not catalog_path.is_file():
        return [
            GovernorFinding(
                ...
                status="fail",
                severity="warn",
                message="the golden-corpus catalog is unreadable",
                observed=(
                    f"catalog={_GOLDEN_CORPUS_CATALOG_REL} present=false "
                    "(lifecycle [bounded]; process anti-drift; not product readiness)."
                ),
                remediation=remediation,
                rule_id="golden-corpus-catalog.missing",
                expected=expected,
    ...
    scalars = _golden_corpus_top_scalars(text)
    problems: list[str] = []
    evidence = [GovernorEvidence(path=_GOLDEN_CORPUS_CATALOG_REL)]
    ...
    lifecycle, lifecycle_line = scalars.get("lifecycle", ("", None))
    authoritative, authoritative_line = scalars.get("authoritative", ("", None))
    if schema_version.strip("\"'") != _GOLDEN_CORPUS_SCHEMA:
    ...
    tiers = tuple(_golden_corpus_dash_items(text, "tiers", "groups"))
    if tiers != _GOLDEN_CORPUS_TIERS:
        problems.append(f"tiers={','.join(tiers) or 'missing'}")

    groups = tuple(_golden_corpus_dash_items(text, "groups", "statuses"))
    if groups != _GOLDEN_CORPUS_GROUPS:
        problems.append(f"groups={','.join(groups) or 'missing'}")

    coverage = _golden_corpus_coverage_map(text)
    if coverage != _GOLDEN_CORPUS_COVERAGE:
        observed_cov = ",".join(f"{name}:{coverage.get(name, 0)}" for name in _GOLDEN_CORPUS_GROUPS)
        problems.append(f"coverage={observed_cov or 'missing'}")

    cases = _golden_corpus_cases(text)
    ids = [str(case.get("id") or "") for case in cases]
    if len(cases) != _GOLDEN_CORPUS_CASE_COUNT:
        problems.append(f"case_count={len(cases)}")
    expected_ids = [f"GC-{index:03d}" for index in range(1, _GOLDEN_CORPUS_CASE_COUNT + 1)]
    if ids != expected_ids:
        problems.append("ids-not-gc-001-040-in-order")
    if len(ids) != len(set(ids)):
        problems.append("ids-not-unique")

    group_counts: dict[str, int] = {name: 0 for name in _GOLDEN_CORPUS_GROUPS}
    allowed_deps = _golden_corpus_allowed_deps(text)
    for case in cases:
        case_id = str(case.get("id") or "?")
        group = str(case.get("group") or "")
        if group not in _GOLDEN_CORPUS_GROUPS:
            problems.append(f"group-outside-closed-set={case_id}:{group}")
        else:
            group_counts[group] += 1
        tier = str(case.get("tier") or "")
        if tier not in _GOLDEN_CORPUS_TIERS:
            problems.append(f"tier-outside-closed-set={case_id}:{tier}")
        status = str(case.get("status") or "")
        if status != "design-only":
            problems.append(f"status-not-design-only={case_id}:{status}")
        deps = list(case.get("deps") or [])
        if not deps:
            problems.append(f"deps-empty={case_id}")
        for dep in deps:
            if dep not in allowed_deps:
                problems.append(f"dep-outside-allowed={case_id}:{dep}")
            elif not (root / dep).is_file():
                problems.append(f"dep-missing={case_id}:{dep}")

    if group_counts != _GOLDEN_CORPUS_COVERAGE:
        observed_groups = ",".join(f"{name}:{group_counts[name]}" for name in _GOLDEN_CORPUS_GROUPS)
        problems.append(f"group-counts={observed_groups}")
    ...
    if problems:
        ...
        return [
            GovernorFinding(
                ...
                severity="warn",
                message=(
                    "the golden-corpus catalog is missing, drifted, or names a "
                    "dep that does not exist"
                ),
                observed=(
                    f"catalog={_GOLDEN_CORPUS_CATALOG_REL} problems={len(problems)} "
                    f"[{preview}] (lifecycle [bounded]; process anti-drift; "
                    "not product readiness)."
                ),
                remediation=remediation,
                rule_id="golden-corpus-catalog.integrity",
                expected=expected,
                evidence=tuple(evidence),
            )
        ]
    ...
    return [
        GovernorFinding(
            ...
            severity="ok",
            message=(
                "golden-corpus catalog headers, closed tiers, unique ids, "
                "group coverage 8/6/6/5/9/6 and existing deps hold"
            ),
            observed=(
                f"catalog={_GOLDEN_CORPUS_CATALOG_REL} case_count={len(cases)} "
                "tiers=G0,G1,G2,G3 coverage=8/6/6/5/9/6 "
                f"unique_ids={len(set(ids))} "
                "(lifecycle [bounded]; process anti-drift; not product readiness)."
            ),
            remediation="none",
            rule_id="golden-corpus-catalog.ok",
            expected=expected,
            evidence=tuple(evidence),
        )
    ]
...
_MODEL_CRYSTAL_INV_RE = re.compile(
    r"^\|\s*INV-(?P<num>\d{2}[a-z]?)(?!\w)",
    re.MULTILINE,
)
_MODEL_CRYSTAL_EXPECTED_INV = frozenset(
    {
        "01",
        "02",
        "03",
...
GOVERNOR_CHECK_SPECS: tuple[CheckSpec, ...] = (
    ...
    _check_spec(
        ...
        "Verify the review-26 section 10 golden-corpus catalog exists with schema_version, lifecycle [proposed], authoritative false, closed tiers G0-G3, unique GC-001..GC-040 ids, group coverage 8/6/6/5/9/6 and deps that name existing companion YAML; process anti-drift, not case execution and not product readiness.",
        (
