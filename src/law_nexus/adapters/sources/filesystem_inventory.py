...
def build_parser_fixture_inventory(root: Path) -> dict[str, Any]:
...
class FilesystemParserFixtureInventory:
    """Filesystem-backed parser fixture inventory adapter.

    [bounded] adapter-layer implementation. It performs repository filesystem
    discovery and source-shape inspection, so it intentionally lives outside
    the domain and application layers.
    """

    def build_parser_fixture_inventory(self, root: Path) -> dict[str, Any]:
        """Build the parser fixture inventory for ``root``."""
        return build_parser_fixture_inventory(root)


# Backward-compatible function name for legacy script tests and wrappers.
build_inventory = build_parser_fixture_inventory
