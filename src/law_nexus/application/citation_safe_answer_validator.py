"""Citation-safe answer validator use case.

[bounded] M076 S13 application seam for deterministic retrieval output
validation. The validator checks citation/evidence shape and emits safe
structured diagnostics only; it does not claim legal correctness, retrieval
quality, parser completeness, or LLM authority.
"""
...
# S13 package-facing aliases. Keep legacy names above for script compatibility.
CitationSafeDiagnostic = Diagnostic
CitationSafeValidationResult = ValidationResult
CitationSafeFixture = Fixture
build_citation_safe_fixture = build_fixture


class CitationSafeAnswerValidator:
    """Validate retrieval output citation/evidence shape with safe diagnostics."""

    def validate(self, request: CitationSafeAnswerValidationRequest) -> ValidationResult:
        fixture = build_fixture(request.fixture_data, fixture_artifact=request.fixture_artifact)
