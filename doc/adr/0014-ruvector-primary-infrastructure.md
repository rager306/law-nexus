...
**Accepted [proposed]** — decision recorded after functional verification of
**isolated** capabilities against synthetic data.

### Proof ceiling

This ADR's 25 functional checks are **bounded synthetic proof only**:

- Vectors generated by `HashEmbedding` (deterministic placeholder), NOT by
  `USER-bge-m3` ONNX runtime. The 1024d dimension matches, but semantic
  quality is unmeasured.
- Graph nodes are synthetic ("art:ed:0", "art:ed:1") with hand-written
  properties, NOT real parsed articles from `law-source/garant/44-fz.odt`.
- KnowQL PoC has a hand-coded AST → RuVector executor, NOT a parser that
  accepts "FIND ARTICLE 5" syntax from a user.
- Temporal queries work on the synthetic dates "2014-01-01".. "2024-08-01",
  NOT on real edition metadata extracted from Garant/Consultant sources.

This ADR MUST NOT be promoted to `[validated]` until:

1. Real Garant ODT file (e.g., `44-fz.odt`) is parsed by the ODT adapter
   into the shared `ParsedBlock` domain type.
2. Parsed text is embedded by `USER-bge-m3` ONNX model (not HashEmbedding)
   and stored in RVF.
3. A real KnowQL query (not the PoC AST) returns a result traceable to the
   original `.odt` file with byte offset.
4. Citation contract is enforced: returned text matches source bytes exactly.

These gates correspond to R035 (bounded evidence before production claims)
and the M111 anti-drift principle: bounded synthetic proof must not be
promoted without real-document proof.

Moves to `[bounded]` when gates 1–3 pass. Moves to `[validated]` when gate 4
passes.
...
- RuVector integration is `[proposed]` — 25 functional checks are bounded
  synthetic proof only (see Proof ceiling above).
...
- HashEmbedding is a deterministic placeholder; USER-bge-m3 ONNX integration
  is NOT verified.
- KnowQL PoC has no real parser; only AST → executor is tested.
- Temporal queries are tested on synthetic dates, not real edition metadata.
